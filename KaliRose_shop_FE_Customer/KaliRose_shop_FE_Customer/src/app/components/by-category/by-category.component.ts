...
export class ByCategoryComponent implements OnInit {
  ...
  getAvgRateProductNew(id: number): number {
    let avgRating: number = 0;
    this.countRate = 0;
    for (const item of  this.rates) {
      if (item.product.productId === id) {
  ...
  sort(keyF: string) {
    if (keyF === 'enteredDate') {
      ...
      if (keyF === 'priceDesc') {
        this.key = '';
        this.products.sort((a,b)=> b.price*(1 - b.discount/100)  -a.price*(1 - a.discount/100)-a.price);
      } else
        if (keyF === 'priceAsc') {
          this.key = '';
          this.products.sort((a,b)=> b.price*(1 - b.discount/100)  -a.price*(1 - a.discount/100)-b.price);
        }
        else {
          this.key = '';
          this.getProducts();
  ...
  // }
  getAllProductLatest() {
    this.productService. getListProductNewest(5).subscribe(data => {
