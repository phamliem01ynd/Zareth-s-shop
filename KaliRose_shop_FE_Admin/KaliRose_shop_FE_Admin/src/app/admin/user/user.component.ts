...
export class UserComponent implements OnInit {
  ...
  columns: string[] = [ 'userId', 'image','name', 'email','address', 'phone', 'gender', 'registerDate', 'view','delete'];
  ...
  ngOnInit(): void {
    ...
    this.pageService.setPageActive('nhanvien');
    this.getAll();
  }
  ...
  getAll() {
    this.customerService.getAll().subscribe(data => {
      ...
      this.customers = this.customers.filter(c=>c.roles[0].name == 'ROLE_ADMIN');
